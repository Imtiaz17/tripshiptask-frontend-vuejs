<template>
    <div style="padding-right:15px;" id="element">
                                        ...
                                        <div class="post">
                                            ...
                                            <div class="post-content">
                                                ...
                                                <div class="columns carryelements">
                                                    <div class="column">
                                                        <label class="label" style="float:left">Pickup Point: </label>
                                                        <span> {{bid.pickup}}</span>
                                                    </div>
                                                    <div class="column">
                                                        <label class="label" style="float:left">Pickup date: </label>
                                                        <span>{{bid.pickup_date}}</span>
                                                    </div>
                                                    <div class="column">
                                                        <label class="label" style="float:left">Delivery date & time: </label>
                                                        <span>{{moment(bid.delivery_date_time).format("lll")}}</span>
                                                    </div>
                                                    <div class="column">
                                                        <label class="label" style="float: left;">Package type</label>
                                                ...
                                                <div class="columns carryelements">
                                                    ...
                                                    <div class="column">
                                                        <label class="label" style="float:left">Pickup time: </label>
                                                        <span>{{bid.pickup_time}}</span>
                                                    </div>
                                                    <div class="column">
                                                        <label class="label" style="float:left">Goods: </label>
                                                        <span>{{bid.goods}}</span>
                                                    </div>
                                                    <div class="column">
                                                        <label style="float:left" class="label">Length x width x height: </label>
                                                        <span>{{bid.length}}x{{bid.width}}x{{bid.height}}</span>
                                                    </div>
                                                </div>
                                                <!-- bid panel -->
                                                <div class="columns carryelements">
                                                    ...
                                                    <div class="column is-4" v-if="bid.ship_owner==id && bid.accepted==1">
                                                        ...
                                                        <div v-else-if=" bid.paid==1" style="float:right">
                                                            ...
                                                            <div style="margin-top:12px;    margin-left: 17%;" v-if="bid.feedback_given==0">
                                                                <a @click="carrierFeedbackModal(index,bid.bidder_name)" class="button btn-dash success-btn btn-dash is-raised rounded ripple  btn-fade">Give a rating</a>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="column is-4" v-if="bid.user_id==id">
                                                    ...
                                                    <!--  <div class="column is-4 is-offset-2" v-if="bid.ship_owner==id">
                                                        <span v-if="bid.accepted==1 && bid.pay==0" class="ongoing">Ship Onging</span>
                                                        <div v-else-if="bid.accepted==1 && bid.pay==1">
                                                            <span class="ongoing">Shipping completed
                                                            </span>
                                                            <div style="margin-top:12px;" v-if="bid.feedback_given==0 ">
                                                                <a @click="shipGiverReviewModal(index,bid.bidder_name) " class="button btn-dash success-btn btn-dash is-raised rounded ripple  btn-fade">Give a rating</a>
                                                            </div>
                                                        </div>
                                                    </div> -->
                                                </div>
        ...
        <div v-for="(bidd,index) in bidsData.carryBids" :key="index" v-if="paymentActiveContent(index)">
        ...
        <div v-for="(bidd,index) in bidsData.carryBids" :key="bidd.id" v-if="carrierActiveModal(index)">
            <vs-popup class="holamundo" :title="title" :active.sync="popupCarrierModal">
                <div class="columns">
                    <div class="column is-4">
                        <label class="label">Give a rating to the carrier</label>
                        <star-rating :show-rating=true :star-size="20" style="font-size:17px;margin-top: 0px;" :increment="0.5" v-model="carrierRating"></star-rating>
                    </div>
                    <div class="column is-8">
        ...
        <!-- feedback to the shipper by carrier -->
        <!--  <div v-for="(bidd,index) in bidsData.carryBids" :key="bidd.user_id" v-if="paymentActiveContent(index)">
            <vs-popup class="holamundo" :title="title" :active.sync="paynReview">
                <div class="columns">
                    <div class="column is-4">
                        <label class="label">Rating</label>
                        <star-rating :show-rating=true :star-size="30" style="font-size:17px;margin-top: 0px;" :increment="0.5" v-model="rating"></star-rating>
                    </div>
                    <div class="column is-8">
                        <textarea class="textarea is-primary-focus" v-model="feedback" rows="5" placeholder="Review about the Rider ..."></textarea>
                    </div>
                </div>
                <div class="columns">
                    <div class="column">
        ...
        <div v-for="(bidd,index) in bidsData.carryBids" :key="bidd.bidder_name" v-if="modalActiveContent(index)">
...
export default {
    ...
    data() {
        return {
            ...
            shipGiverReviewModalActive: false,
            ownerTitle: "",
            ownerRating: 0,
            ownerFeedback: '',
    ...
    mounted() {
        EventBus.$on('Carryingbid', (data) => {
            this.bidsData = data
            this.$scrollTo(carryelement, 1000, options)
        })
    ...
    methods: {
        ...
        //     return (this.title = ('Give rating and payment to ').concat(name), this.activePay = i)
        // },

        shipGiverReviewModal(i, name) {
            this.shipGiverReviewModalActive = true;
            return (this.ownerTitle = ('Give rating and feedback to ').concat(name), this.activeReview = i)
        },
        taskOwnerReviewontent: function(i) {
        ...
        accept(id) {
            this.isLoading = true
            this.$axios.patch(`carryshipbids/${id}`, {
        ...
        shipperReview(id, carrierid) {
            ...
            this.isLoading = true
            this.$axios.post('shipseekersfeedback', {
                    carrierId: carrierid,
